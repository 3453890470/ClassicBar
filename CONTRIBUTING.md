...
相同或高度相似的代码片段出现 **≥2 次** 时，应抽取为可复用的方法或工具类。
...
典型场景：
- try/catch 包裹的 MagiCap 读取（`getCurrentMana()` + `getMaxMana()`）
- 重复的 `Color.reset()` + `renderFullBarBackground()` + `applyConfiguredBarColor()` 三段式
- 相同结构的 `textHelper(graphics, xStart, yStart, ...)` 参数拼接

示例：

```java
// ❌ 重复 3 次
double barWidthA = getBarWidth(playerA);
double barWidthB = getBarWidth(playerB);
double barWidthC = getBarWidth(playerC);

// ✅ 抽取循环或批量方法
for (var p : players) {
    double w = getBarWidth(p);
    processBar(w);
}
```

不适用场景：
- Builder/fluent API 中链式调用的重复是模式的一部分，不抽取
- 测试代码中 Arrange-Act-Assert 的重复上下文，除非超过 5 处
