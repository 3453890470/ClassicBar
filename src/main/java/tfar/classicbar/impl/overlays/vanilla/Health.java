...
public class Health extends BarOverlayImpl {
    ...
    public void renderBar(HudRenderContext context, GuiGraphicsExtractor graphics, Player player, int screenWidth, int screenHeight, int vOffset) {
        ...
        playerHealth = health;
        double displayHealth = health + (lastPlayerHealth - health) * ((double) player.invulnerableTime / Math.max(player.invulnerableTime, 20));
        ...
        if (hasPoison) {
            ...
            if (c != null) {
                c.color2Gla(ConfigCache.healthPoisonOverlayAlpha);
        ...
        if (hasWither) {
            ...
            if (c != null) {
                c.color2Gla(ConfigCache.healthWitherOverlayAlpha);
                ModUtils.drawTexturedModalRect(graphics, f + 1, yStart + 1, 1, 36, barWidth, 7);
                Color.reset();
            }
        }
        if (hasFrozen) {
            ...
            if (c != null) {
                c.color2Gla(ConfigCache.healthFrozenOverlayAlpha);
                ModUtils.drawTexturedModalRect(graphics, f + 1, yStart + 1, 1, 36, barWidth, 7);
                Color.reset();
            }
        }
    ...
    public void renderText(GuiGraphicsExtractor graphics, Player player, int width, int height, int vOffset) {
        ...
        // textX anchors to baseX (same as the first icon slot).
        // textHelper internally handles the icon-width offset (+9 for RHS, -9 for LHS).
        int textX = baseX;
